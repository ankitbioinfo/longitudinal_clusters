...
%datafile='DF/';
datafile='plothist_postnatal/PT/';
%datafile='plothist_postnatal/DF/';
...
columnlike=zeros(1,11);
clusterlike=zeros(1,11);
...
for i=1:length(LCC)
    ...
    if length(LCC{i})>10
        if find_real_columns_or_not(LCC{i},data_EL_conn_comp,cutoff)
            columnlike(11)=columnlike(11)+1;
            
        else
            clusterlike(11)=clusterlike(11)+1;
        end
    end
    ...
    if length(LCC{i})==7
         if find_real_columns_or_not(LCC{i},data_EL_conn_comp,cutoff)
                columnlike(7)=columnlike(7)+1;
         else
                clusterlike(7)=clusterlike(7)+1;
         end
    end
    
    if length(LCC{i})==8
          if find_real_columns_or_not(LCC{i},data_EL_conn_comp,cutoff)
                columnlike(8)=columnlike(8)+1;
          else
                clusterlike(8)=clusterlike(8)+1;
          end
    end
    
    if length(LCC{i})==9
         if find_real_columns_or_not(LCC{i},data_EL_conn_comp,cutoff)
            columnlike(9)=columnlike(9)+1;
         else 
            clusterlike(9)=clusterlike(9)+1;
         end
    end
    
    if length(LCC{i})==10
         if find_real_columns_or_not(LCC{i},data_EL_conn_comp,cutoff)
             columnlike(10)=columnlike(10)+1;
             
         else
             clusterlike(10)=clusterlike(10)+1;
         end
    end
