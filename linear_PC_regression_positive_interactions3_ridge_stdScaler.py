...
def find_PC_of_invidualCluster_in_SC(scbarcode,scadata,no_of_pc,spbarcode,spadata, sct_ad_sc_full):
    ...
    #print('sc',sc_cellname,len(sc_cellname))
    #print('sp',sp_cellname,len(sp_cellname))

    sc.pp.pca(sct_ad_sc,n_comps=no_of_pc)
    sc_com_pc=sct_ad_sc.varm['PCs']

    #sc.pp.pca(full_genes_sc,n_comps=no_of_pc)
    #sc_HVG_pc=full_genes_sc.varm['PCs']
    #print(full_genes_sc)
    #print(full_genes_sc.X.shape)
    ...
    M=find_correlation_bw_genes_and_PC_component_in_singlecell(transfer_sc_com,CbyG)
    #print(sc_com_pc.shape,msc.shape,msp.shape)
    #print(transfer_sp_com.shape,transfer_sc_com.shape)
    ...
    return transfer_sp_com, transfer_sc_com, sp_barcode,sc_barcode, M,sc_genenames
    #return transfer_sp_com, transfer_sc_com, sp_barcode,sc_barcode, sc_HVG_pc,sc_genenames
...
def compute_PC_space(input,sct_ad_sc_full):
    ...
    #print('common cell types',len(common))
    ...
    for i in range(n):
        ...
        #print(i,input.spatialcell_unique_clustername[i],len(spbarcode),len(scbarcode))
        pc_sp,pc_sc,sp_barcode,sc_barcode,sc_HVG_pc,sc_genenames=find_PC_of_invidualCluster_in_SC(scbarcode,input.ad_sc,input.no_of_pc,spbarcode,input.ad_sp, sct_ad_sc_full)
        print(i,pc_sp.shape,pc_sc.shape,sc_HVG_pc.shape,sc_genenames.shape)
        PCA_of_sc_cluster_accordingto_spatial_clusterid[clidsp]=[sc_HVG_pc,sc_genenames]
...
#def model_linear_regression(K_fold,n_repeats,neighborhoodClass,target,lambda_c,strategy,BothLinearAndCrossTerms,seed,n_jobs):
    ...
    for i in range(len(input.spatialcell_unique_clusterid)):
        ...
        #print(input.spatialcell_unique_clustername,input.spatialcell_unique_clusterid)
        neighborhoodClass=neighborhoodClass[:,newindex]
        #print(i,neighborhoodClass.shape,target.shape)
        xlabel=np.array(xlabel)
        score=np.array(score)

        count+=neighborhoodClass.shape[0]
        coef,intercept,alpha,percent_variance_explained,residual_variance_explained=run_ridge_regression(input.seed ,input.lambda_c,input.K_fold,input.n_repeats,target,neighborhoodClass)
        ...
        np.savez(savedata,coef_mu=coef,intercept=intercept,alpha=alpha,xlabel=xlabel,score=score,pve=percent_variance_explained,rve=residual_variance_explained)
...
def run_ridge_regression(seed,lambda_c,K_fold,n_repeats,target,neighborhoodClass):
    seed=seed+1
    ...
    while(flag):
        seed=seed+1
        sss = RepeatedKFold(n_splits=K_fold, n_repeats=1 ,random_state=seed)
        gs_grid = GridSearchCV(linear_model, parameters, scoring='r2',cv=sss,n_jobs=n_jobs)
        pipe_grid=Pipeline([  ('polynomial_features',polynomial),('StandardScaler',StandardScaler()), ('linear_regression_grid',gs_grid)])
    ...
    seed=seed+1
    #print("kfold",K_fold,n_repeats)
    #K_fold=1
    #n_repeats=1
    #sss = RepeatedKFold(n_splits=K_fold, n_repeats=n_repeats ,random_state=seed)
    #for train_index, test_index in sss.split(neighborhoodClass,target):
    train_index=range(target.shape[0])
    test_index=[]

    if True:
        #print(len(train_index),len(test_index),neighborhoodClass.shape)
        ...
        pipe=Pipeline([  ('polynomial_features',polynomial), ('StandardScaler',StandardScaler()),('ridge_regression',linear_model)])

        pipe.fit(x_train, y_train)

        #print(x_test.shape,x_train.shape,y_pred.shape)
        '''
        print('11',explained_variance_score(y_test, y_pred))
        print('22',explained_variance_score(y_test, y_pred,multioutput='raw_values'))
        print('33',explained_variance_score(y_test, y_pred,multioutput='variance_weighted'))
        print('44',explained_variance_score(y_test, y_pred,multioutput='uniform_average'))

        print('11',r2_score(y_test, y_pred))
        print('22',r2_score(y_test, y_pred,multioutput='raw_values'))
        print('33',r2_score(y_test, y_pred,multioutput='variance_weighted'))
        print('44',r2_score(y_test, y_pred,multioutput='uniform_average'))
        ...
        '''
        #r2_score,mean_absolute_error,mean_squared_error,mean_squared_log_error,mean_absolute_percentage_error,
        #median_absolute_error, max_error, explained_variance_score,d2_absolute_error_score

        '''
        y_pred=pipe.predict(x_test)
        scorecalc[0].append(pipe.score(x_test, y_test))
        scorecalc[1].append(r2_score(y_test, y_pred,multioutput='variance_weighted'))
        scorecalc[2].append(r2_score(y_test, y_pred,multioutput='uniform_average'))
        scorecalc[3].append(mean_squared_error(y_test, y_pred))
        scorecalc[4].append(mean_absolute_error(y_test, y_pred))
        scorecalc[5].append(explained_variance_score(y_test, y_pred,multioutput='variance_weighted'))
        scorecalc[6].append(explained_variance_score(y_test, y_pred,multioutput='uniform_average'))
        '''
    ...
    coef_mu=np.mean(np.array(coef),axis=0)

    comp_score_std=np.std(np.array(scorecalc),axis=1)
    comp_score=np.mean(np.array(scorecalc),axis=1)

    #print('training',x_train.shape,'testing',x_test.shape,'coeff',coef_mu.shape,'Iteration',len(scorecalc[0]))

    CTFeatures=poly.get_feature_names()
    #print("Features", CTFeatures)
    print("accuracy score\t",np.mean(scorecalc[0]))
    print("r2_score\t",np.mean(scorecalc[1]))

    print("\nmean_absolute_error\t",np.mean(scorecalc[2]))
    print("mean_squared_error\t",np.mean(scorecalc[3]))
    print("mean_absolute_percentage_error\t",np.mean(scorecalc[4]))

    print("\nmedian_absolute_error\t",np.mean(scorecalc[5]))
    print("max_error\t",np.mean(scorecalc[6]))
    print("\nexplained_variance_score\t",np.mean(scorecalc[7]))
    #print("d2_absolute_error_score\t",np.mean(scorecalc[9]))
    return coef_mu,comp_score,coef_std,comp_score_std,lambda_c,percent_variance_explained,residual_variance_explained

    '''
    #print('%',percent_variance_explained,residual_variance_explained)

    return coef,intercept,lambda_c,percent_variance_explained,residual_variance_explained
...
def main():
    inputRadius=[25]
    input_spatial='input_vizgen_liver_zcorrect6_Res2/spatial/'
    input_sp_dir='input_vizgen_liver_zcorrect6_Res2/data/'
    input_sc_dir='input_vizgen_liver_zcorrect6_Res2/scdata/'
    no_of_pc=2


    # load single cell data
    fname=input_sc_dir+'scTransform_singleCell.h5ad'
    sct_ad_sc=sc.read_h5ad(fname)
    full_ad_sc=sc.read_h5ad(input_sc_dir+'sc_liver_data_HVG.h5ad')
    '''
    full_ad_sc=sc.read_h5ad(input_sc_dir+'sc_liver_data.h5ad')
    print('full',full_ad_sc)
    sc.pp.normalize_total(full_ad_sc, inplace=True)
    sc.pp.log1p(full_ad_sc)
    print('1',full_ad_sc)
    sc.pp.highly_variable_genes(full_ad_sc, flavor="seurat", n_top_genes=2000)
    #sc.pl.highly_variable_genes(full_ad_sc,show=False,save='.png')
    print('2',full_ad_sc)
    full_ad_sc = full_ad_sc[:, full_ad_sc.var.highly_variable]
    print('3',full_ad_sc)
    full_ad_sc.write_h5ad(input_sc_dir+'sc_liver_data_HVG.h5ad')
    '''


    name=input_sc_dir+'annot_mouseStStAll_correct_monocytes_hep1234.csv'
    df=pd.read_csv(name)
    data=df.to_numpy()
    ...
    # load spatial dat
    fname=input_sp_dir+'scTransform_spatial.h5ad'
    ...
    #print(singlecell_unique_clustername)
    #print(spatialcell_unique_clustername)
    ...
    for radius in inputRadius:
        ...
        neighbors=pickle.load( open(input_spatial+'save_neighbors_'+str(radius)+'.p', "rb" ) )
        maindir=input_spatial+'/PCregression'+str(radius)+'/'
        ...
        fname=input_spatial+'L2_multi_linear/save_numpy_array_'+str(radius)+'.npz'
        data=np.load(fname)
        ...
        for i in range(len(neighbors)):
            l+=len(neighbors[i])
        print('avg neighbors',l/len(neighbors))
        fw=open(maindir+'/prediction_R'+str(radius)+'.csv','w')
        workbook = xlsxwriter.Workbook(maindir+'/PCregression_R'+str(radius)+'.xlsx')
        #my_worksheets = {ws.get_name(): ws for ws in workbook.worksheets()}
        #print('my name',my_worksheets)
        #worksheet = workbook.get_worksheet_by_name(worksheetName)
        worksheet = workbook.add_worksheet('thres 2')
        worksheetrow=0
        #fw.write('\nRadius = '+ str(radius)  + '\n')


        #fw.write('CC\tNC\tLigand\tReceptor\t# of loca pairs\t#pairs > threshold\t prob \tlig thres\trec thres\tmean\tvar\tmean\tvar\n')
        header=['CC(A)','NC(B)','localized score','Ligand','Receptor','beta0','beta1','intercept']
        #'thres(L)','thres(R)', 'H(AB>L|AnB>L)','H(AB>L|nAB>L)','H(AB>L|nAnB>L)',
        #'H(AB>R|AnB>R)','H(AB>R|nAB>R)','H(AB>R|nAnB>R)','Database',
        #'pv AB|AnB','pv AB|nAB','pv AB|nAnB',
        #'P(A>L,B>R)','P(A>L,nB>R)' , 'P(nA>L,B>R)', 'P(nA>L,nB>R)',
        #'P(A>L,B>R)/E[A>L]E[B>R]','P(A>L,nB>R)/E[A>L]E[nB>R]','P(nA>L,B>R)/E[nA>L]E[B>R]','P(nA>L,nB>R)E[nA>L]E[nB>R]',
        #'#AB','#AnB','#nAB','#nAnB', '#(A>L,B>R)','#(A>L,nB>R)','#(nA>L,B>R)','#(nA>L,nB>R)']
        # 'nB:E[R]', ] #'nB:mu(R)','nB:var(R)',
        for i in range(len(header)):
            worksheet.write(worksheetrow,i,header[i])
        worksheetrow+=1
        ...
        #inputdata['n_jobs']=-1 #no of processors For details see here https://scikit-learn.org/stable/glossary.html#term-n_jobs
        inputdata['lambda_c']=list(np.power(2.0, np.arange(-24, 24))) # inverse of lambda regularization
        #BothLinearAndCrossTerms=1# If only linearterms then put 1; For both linear and crossterms use 2
        inputdata['K_fold']=5 #no of cross folds
        inputdata['seed']=3685134
        inputdata['n_repeats']=1 #no of times to run the logistic regression after finding the hyperparameters
        ...
        find_canonical_pathways_in_interacting_cell_types(input,maindir,savedir,logistic_coef_cutoff)

        #find_correlation_PC_plots_of_some_eg(input,maindir)


def find_correlation_PC_plots_of_some_eg(input,maindir):
        savecorrelation=maindir+'/scatterPlot'+str(input.no_of_pc)+'/'
        #savecorrelation2=maindir+'/CorrelationNH'+str(input.no_of_pc)+'/'
        create_directory(savecorrelation)
        #create_directory(savecorrelation2)
        CC_eg=['pDCs','Central Vein EC','Central Vein EC','Portain Vein EC','Portain Vein EC',
        'LSECs','Stellate cells','Stellate cells','Lymphatic EC','Lymphatic EC',
        'cDC2s','cDC2s','cDC2s','pDCs','pDCs',
        'B cells','B cells','B cells','cDC1s',
        ...
        NC_eg=['Hep1245','Hep3678','Fibroblast','Cholangiocytes','Fibroblast',
        'Hep1245','Hep1245','Hep3678','Fibroblast','Lymphatic EC',
        'Cholangiocytes','Hep3678','LSECs','Hep1245','Fibroblast',
        'LSECs','Monocytes','Mig. cDCs','LSECs',
        'KCs','Lymphatic EC','Stellate cells','LSECs']
        NCPC=[3,5,1,2,1,
        3,3,4,5,5,
        1,1,3,3,1,
        4,3,3,3,
        ...
        for j in range(len(CC_eg)):
            ...
            for i in range(len(gene)):
                    ax.text(first[i],second[i],gene[i],fontsize=5)
            #plt.scatter(first[index],second[index],c='blue')

            ax.scatter(first,second,s=1,marker='o',c='b')
            ...
            if j==9:
                ...
                for i in range(len(first)):
                    fw.write(first[i]+'\t'+second[i]+'\n')

            print(j+1,np.dot(first,second))
            '''

                #print(first[i],second[i],prod)
            ...
            fig.savefig(savecorrelation+'corr'+str(j+1)+'.png',dpi=300)
            plt.close('all')
            #fig.clf()


def find_canonical_pathways_in_interacting_cell_types(input,maindir,savedir,logistic_coef_cutoff):

    coeff_cutoff_for_log_reg=0
    #coeff_cutoff_for_log_reg=logistic_coef_cutoff
    coeff_cutoff_for_rid_reg=0.5

    PCA_of_sc_cluster_accordingto_spatial_clusterid=pickle.load(open(maindir+'PCA_of_sc_cluster'+str(input.no_of_pc)+'.p', 'rb'))
    n=len(input.spatialcell_unique_clustername)


    workbook = xlsxwriter.Workbook(maindir+'/up_and_down_genes_'+str(input.no_of_pc)+'.xlsx')
    ...
    for i in range(n):
        ...
        for j in range(input.no_of_pc):
            ylabelname.append('CC_'+filename+'_PC'+str(j+1))
            componentlabel.append('PC'+str(j+1))

        #bigmat=PCA
        #print('god',len(ylabelname),bigmat.shape,np.min(bigmat),np.max(bigmat),len(temp))
        for k in range(len(xlabel)):
            if score[k]>coeff_cutoff_for_log_reg:
                if filename!=xlabel[k]:
                    NH_PCA,gene=PCA_of_sc_cluster_accordingto_spatial_clusterid[d[xlabel[k]]]
                    #bigmat=np.hstack((bigmat,NH_PCA))
                    #print('min max',np.min(NH_PCA),np.max(NH_PCA))
                    for j in range(input.no_of_pc):
                        ylabelname.append('NC_'+xlabel[k]+'_s'+'%0.3f'%score[k]+'_PC'+str(j+1))

        NPClabelindex=[]
        for k in range(len(xlabel)):
            for j in range(input.no_of_pc):
                NPClabelindex.append(j)

        #print(filename,coef_mu.shape,score.shape,len(ylabelname))
        fout.write(filename+'\n')
            ...
            for j in range(normalized_ridge_coef.shape[1]):
                ...
                if abs(normalized_ridge_coef[k,j])>coeff_cutoff_for_rid_reg:
                    if score[index]>coeff_cutoff_for_log_reg:
                        NH_PCA,gene=PCA_of_sc_cluster_accordingto_spatial_clusterid[d[xlabel[index]]]
                        top_genes_in_CC,top_genes_in_NC,genesWithUP,genesWithDown=find_fold_change(PCA,NH_PCA,gene,k,NPClabelindex[j])
                        #print('CC',len(top_genes_in_CC),top_genes_in_CC[0:10],genesWithUP)
                        #print('NC',len(top_genes_in_NC),top_genes_in_NC[0:10],genesWithDown)
                        if len(genesWithUP)>0:
                            name_cc='_CC_'+filename.replace('.','_')+'_PC'+str(k+1)+'_'+xlabel[index].replace('.','_')+ '_PC'+str(1+NPClabelindex[j])+'_IP'+str(count)
                            savename=maindir+'PCcomponent_wise_pathway_figures/'+str(i+1)
                            #pathway_analysis(savename,name_cc,top_genes_in_CC)
                        if len(genesWithDown)>0:
                            name_cc='_NC_'+filename.replace('.','_')+'_PC'+str(k+1)+'_'+xlabel[index].replace('.','_')+ '_PC'+str(1+NPClabelindex[j])+'_IP'+str(count)
                            savename=maindir+'PCcomponent_wise_pathway_figures/'+str(i+1)
                            #pathway_analysis(savename,name_cc,top_genes_in_NC)


                        fout.write('CC-PC'+str(k+1)+'\t'+'%0.3f'%(score[index])+'\tNC-PC'+str(1+NPClabelindex[j])+'\t'+xlabel[index]+'\t%0.3f'%(normalized_ridge_coef[k,j])+'\t'+str(count))
                        fout.write('\n')
                        '''
                        genesWithUP=''
                        genesWithDown=''
        ...
        #temp=np.argwhere(np.isnan(bigmat))
        #bigmat[temp[:,0],temp[:,1]]=0
        #temp=np.argwhere(np.isnan(PCA))
        #PCA[temp[:,0],temp[:,1]]=0
        #print(len(componentlabel),PCA.shape)
        #excelfilesave=1
        #find_correlation(input,ylabelname,bigmat,gene,coef_mu,savecorrelation2,score,clid,filename,workbook,excelfilesave)
        #excelfilesave=0
        #find_correlation(input,componentlabel,PCA,gene,coef_mu,savecorrelation,score,clid,filename,workbook,excelfilesave)
    workbook.close()


def pathway_analysis(savename,titlename,g1):
    titlename='Rea'+titlename
    finalsavename=savename+titlename
    gene_set_names = gseapy.get_library_name(organism='Mouse')
    #database=['GO_Biological_Process_2021','Reactome_2016','HDSigDB_Mouse_2021','KEGG_2019_Mouse','Mouse_Gene_Atlas','RNAseq_Automatic_GEO_Signatures_Mouse_Down', 'RNAseq_Automatic_GEO_Signatures_Mouse_Up','WikiPathways_2019_Mouse']
    ...
    for i in range(len(database)):
        enr_res1 = gseapy.enrichr(gene_list=g1,organism='Mouse',gene_sets=database[i],description='pathway',cutoff = 0.5)
        finalsavename.replace(' ','_')
        gseapy.barplot(enr_res1.res2d,title=titlename,ofname=finalsavename)#database[i]+titlename




def find_fold_change(PCA,NH_PCA,gene,CCPC,NCPC):
    ...
    for i in range(len(ind2)):
        if abs(second[ind2[i]])>0.1:
            ...
            if gene[ind2[i]] in listofallLR:
                nc_genes2.append([gene[ ind2[i] ],'%0.2f'%second[ ind2[i] ]])

    return cc_genes, nc_genes,cc_genes2,nc_genes2

'''
def find_correlation(input,xlabel,PCA,gene,coef,savedir,score,clid,filename,workbook,excelfilesave):
    ...
    if True:
        ...
        nM=M
        nG=gene
        yaxisscale=(len(xlabel)/5 -1)*1 + 4
        #yaxisscale=5
        #xaxisscale=2*len(nG)/10
        xaxisscale=16
        fig,axs=plt.subplots(1,1,figsize=(xaxisscale,yaxisscale))
        b=snn.heatmap(nM.T,yticklabels=xlabel)# ,xticklabels=nG
        _, xlabels= plt.xticks()
        b.set_xticklabels(xlabels,size = 5)
        #plt.title('CC:'+filename+', NC:'+xlabel[k]+', normalized score = %0.3f'%score[k])
        plt.title(filename)
        fig.tight_layout()
        fig.savefig(savedir+'corr_regcoef_PCA_'+str(clid)+'_'+str(filename)+'.png',dpi=300)
        fig.clf()

    #fig,axs=plt.subplots(1,1,figsize=(xaxisscale,yaxisscale))
    df = pd.DataFrame(nM.T, columns=nG,index=xlabel)
    b=snn.clustermap(df,vmin=-1,vmax=1)#,cbar_pos=(0, 1, .01, .4))
    #b=snn.clustermap(df)
    rowind=b.dendrogram_row.reordered_ind
    colind=b.dendrogram_col.reordered_ind
    ...
    plt.savefig(savedir+'biclustering_'+str(clid)+'_'+str(filename)+'.png',dpi=300)
    plt.close('all')

    if excelfilesave==1:
        worksheet = workbook.add_worksheet(filename)
        worksheetrow=0
        worksheet.write(worksheetrow,1,'mean')
        for j in range(len(rowind)):
            worksheet.write(worksheetrow,j+2,xlabel[rowind[j]])

        for i in range(len(nG)):
            worksheetrow+=1
            worksheet.write(worksheetrow,0,nG[colind[i]])
            temp=nM[colind[i],:]
            worksheet.write(worksheetrow,1,np.mean(temp))
            for j in range(len(rowind)):
                worksheet.write(worksheetrow,j+2,temp[rowind[j]])

    return 0


def find_canonical_gene_gene_correlation(input,maindir):
    noclu=len(input.spatialcell_unique_clusterid)
    d={}
    for i in range(noclu):
        index=np.where(input.spatialcell_unique_clusterid[i]==input.annotation_spatial_cluster_id)
        d[input.spatialcell_unique_clusterid[i]]=index[0]


    count=0
    for key in d:
        count=count+len(d[key])
    cellname=input.ad_sp.obs_names.to_numpy()
    gene=input.ad_sp.var_names.to_numpy()

    print(count,len(cellname),len(gene))

    GGP_index=np.zeros((len(gene),len(gene)),dtype=int)
    GGP_name=[]
    count=0
    for i in range(len(gene)):
        for j in range(i+1,len(gene)):
            GGP_index[i,j]=count
            count+=1
            GGP_name.append(gene[i]+'_'+gene[j])

    #n=len(cellname)
    #n=5
    #noclu=1
    M=np.zeros((len(GGP_name),noclu),dtype=float)

    tp_sp=str(type(input.ad_sp.X))
    if tp_sp=="<class 'scipy.sparse.csr.csr_matrix'>":
        msp=input.ad_sp.X.toarray()
    else:
        msp=input.ad_sp.X

    print('cell',len(cellname),'gene',len(gene),msp.shape)
    for k in range(noclu):
        key=input.spatialcell_unique_clusterid[k]
        cellindex=d[key]
        for i in range(len(gene)):
            for j in range(i+1,len(gene)):
                geneindex=GGP_index[i,j]
                corr,_ = pearsonr(msp[cellindex,i],msp[cellindex,j])
                M[geneindex,k]=corr

    df=pd.DataFrame(data=M, index=GGP_name , columns=input.spatialcell_unique_clustername[0:noclu])
    df.to_csv(maindir+"gene_gene_correlation.csv")
'''
...
def plot_results(savedir,maindir,radius,input):
    ...
    for i in range(len(input.spatialcell_unique_clusterid)):
        ...
        b=snn.heatmap(coef_mu,xticklabels=ylabelname,yticklabels=componentlabel)
        _, xlabels= plt.xticks()
        b.set_xticklabels(xlabels,size = 5)
        ...
        plt.tight_layout()
        plt.savefig(savedir+'coeff_matrix_'+str(input.spatialcell_unique_clusterid[i])+'_'+filename+'.png',dpi=300)
        plt.close()
