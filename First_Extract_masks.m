...
for gi=1:length(allpath)
        ...
        outputpath=strcat('MakeListClustersMask/',s{2});    
...
for position = nosec{gi}; %    [3,4,7,11,12]   %coordinates(:,1)'  
    %load(strcat(path,'CreER_masked_nuclei',sprintf('%02d',position),' (Characteristics).mat'));
    sname=s{2}(1:strlength(s{2})-1);
    name=strcat(path,pseudocolor{clone},'_cluster_mask_',sname,'_sec',sprintf('%d',position), ' (Characteristics).mat'); 
    load(name);
